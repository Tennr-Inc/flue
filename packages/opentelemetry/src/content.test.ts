...
it('exports valid message arrays for unshrinkable and unserializable tool histories', async () => {
	...
	try {
		for (const [index, args] of [
			...
			const event: FlueObservation = {
				...
				request: {
					...
					input: {
						systemPrompt: '🙂'.repeat(40_000),
						tools: [
							{ name: 'lookup', description: 'Look up a record.', parameters: { const: args } },
						],
						messages: [
							{
								role: 'assistant',
		...
		for (const span of spans) {
			...
			expect(JSON.parse(span.attributes['gen_ai.tool.definitions'] as string)).toEqual([
				{ type: 'flue', name: '[flue]', description: expect.stringContaining('[flue]') },
			]);
			const system = span.attributes['gen_ai.system_instructions'] as string;
			expect(Buffer.byteLength(system)).toBeLessThanOrEqual(25_000);
			expect(JSON.parse(system)).toEqual([
				{ type: 'text', content: expect.stringContaining('[flue:truncated,') },
			]);
