...
export function assertToolDefinition(
	...
	if (tool.approval !== undefined) {
		if (tool.version === undefined) {
			throw new Error(
				`[flue] ${label} version is required when approval.required is true so an approval cannot authorize changed tool code after a deploy.`,
			);
		}
		if (!isApprovalPolicy(tool.approval)) {
			throw new Error(`[flue] ${label} approval must be { required: true }.`);
		}
		assertKnownFields(tool.approval, TOOL_APPROVAL_FIELDS, `${label} approval`);
		if (
		...
		if (presentation !== undefined) {
			...
			assertKnownFields(
				presentation,
				TOOL_APPROVAL_PRESENTATION_FIELDS,
...
function assertKnownFields(value: object, fields: ReadonlySet<string>, label: string): void {
	for (const key of Object.keys(value)) {
		if (fields.has(key)) continue;
		throw new Error(
			`[flue] ${label} received unknown field "${key}". Accepted fields: ${[...fields].join(', ')}.`,
		);
	}
}
