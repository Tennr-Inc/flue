...
export class Session implements FlueSession, AgentSubmissionSession {
	...
	private async runWouldStopPhase(options: {
		...
		const driveContinuation = async () => {
			...
			if (!this.approvalParked) this.throwIfError(options.errorLabel);
		};
	...
	private async resolveApprovalToolCalls(
		...
		for (const call of approvalCalls) {
			...
			const decisionRecordId = `record_tool_approval_decided_${encodeCanonicalId(approval.proposalId)}`;
			if (!(await this.conversationWriter.hasRecord(decisionRecordId))) {
				await this.appendCanonical([
					{
						...this.canonicalEnvelope('tool_approval_decided', decisionRecordId),
	...
	private async settleTrailingToolBatch(
		...
		const resolved = new Map<string, ConversationRecord>();
		const approvalDecisionRecords: ConversationRecord[] = [];
		...
		for (const toolCall of partial.toolCalls) {
			...
			if (toolDef?.approval?.required || recordedApprovalCalls.has(toolCall.id) || approval) {
				...
				if (approvalMatches && approvalStore?.decideToolApproval) {
					...
					const decisionRecordId = `record_tool_approval_decided_${encodeCanonicalId(decided.proposalId)}`;
					if (!(await this.conversationWriter.hasRecord(decisionRecordId))) {
						approvalDecisionRecords.push({
							...this.canonicalEnvelope('tool_approval_decided', decisionRecordId),
							type: 'tool_approval_decided',
						...
						executionStarted
							...
							: this.approvalFailureOutcomeRecord(
									...
									decided.status === 'aborted'
										? (decided.reason ?? 'Submission aborted; the tool was not executed.')
										: 'Submission aborted before the approved tool call ran; the tool was not executed.',
								),
					);
				} else {
					resolved.set(
		...
		if (approvalDecisionRecords.length > 0) await this.appendCanonical(approvalDecisionRecords);
	...
	private async ensureApprovalRequestedRecord(approval: ToolApproval): Promise<void> {
		const requestedRecordId = `record_tool_approval_requested_${encodeCanonicalId(approval.proposalId)}`;
		if (await this.conversationWriter.hasRecord(requestedRecordId)) return;
		await this.appendCanonical([
			{
				...this.canonicalEnvelope('tool_approval_requested', requestedRecordId),
				type: 'tool_approval_requested',
				proposalId: approval.proposalId,
				submissionId: approval.submissionId,
	...
	private notifyApprovalProvider(approval: ToolApproval): void {
		...
		void Promise.resolve()
			.then(() => getToolApprovalProvider()?.requested(approval))
			.catch((error) => {
	...
	private async resumeConversationToCompletion(options: {
		...
		const stopAtApprovalBoundary = async (): Promise<boolean> => {
			if (!this.approvalParked) return false;
			if (!this.approvalSubmissionParked) {
				await this.throwIfSubmissionHalted();
				// The proposal was already decided before the post-batch park. The
				// current attempt still owns the submission, so repair the canonical
				// batch and continue without manufacturing a waiting transition.
		...
		switch (state.kind) {
			...
			case 'resume': {
				...
				if (await stopAtApprovalBoundary()) return;
