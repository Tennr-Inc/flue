...
describe('chronological transcript', () => {
	...
	it('preserves the release baseline retry content while separating the replacement step', () => {
		...
		expect(readSubmissionReply(live, 'submission')).toEqual(
			readSubmissionReply(snapshot(records), 'submission'),
		);
	});
